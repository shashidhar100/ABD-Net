...
#seg_classes = {'Cone':[2], 'Cylinder':[3], 'Plane':[0], 'Sphere':[1]}
seg_classes = {'Primitive':[1]}
...
def parse_args():
    ...
    parser.add_argument('--epoch',  default=22, type=int, help='Epoch to run [default: 251]')
    ...
    parser.add_argument('--npoint', type=int,  default=8096, help='Point Number [changed to:8096]')
...
def main(args):
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for i, data in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            #print("lskdjflkjdsljfl\n\n")
            
            #seg_pred, trans_feat = classifier(points, to_categorical(label, num_classes))##########
            #seg_pred, trans_feat = classifier(points)
            seg_pred = classifier(points)

            #print("sdfsdjkf:",seg_pred.shape)
            #exit()
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                xxx = seg_pred.contiguous().view(-1, num_classes)
                ttt = target.view(-1, 1)[:, 0]
                ppp = xxx.data.max(1)[1]
                ccc = ppp.eq(ttt.data).cpu().sum()
                m_correct.append(ccc.item() / (args.batch_size * args.npoint))
